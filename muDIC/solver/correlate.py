...
def my_correlate_img_to_ref_q4(node_coordss, img, ref, settings):
    # Instantiate empty arrays
...
def my_correlate(inputs, correlator, reference_gen):
    """
   Main correlation routine
    This routine manages result storage, reference generation and
     the necessary logic for handling convergence issues.
   ...
   node_coords, reference_stack, Ic_stacks
    ...
    try:
        for image_id in range(1, settings.max_nr_im):
            ...
            try:
                node_coords, Ic, conv, koef = correlator(node_coords, img, reference, settings)
        ...
        return np.array(node_position_t), reference_stack, Ic_stacks, koefs
    

    
class MyDICAnalysis(DICAnalysis) :
    def __init__(self, inputs):
    ...
    def __solve__(self):
        ...
        # if isinstance(self.__input__.mesh.element_def, BSplineSurface):
        #     node_position, reference_stack, Ic_Stack, koef = my_correlate(self.__input__, correlate_img_to_ref_spline,
        #                                                          generate_reference)

        # else:
        node_position, reference_stack, Ic_Stack, koefs = my_correlate(self.__input__, my_correlate_img_to_ref_q4,
                                                             generate_reference_Q4)

        # TODO: Remove the need of transposing the matrices
        return node_position[:, 0, :].transpose(), node_position[:, 1,
                                                   :].transpose(), reference_stack, Ic_Stack, koefs
    ...
    def run(self):
        """
         ...
         """
        node_x, node_y, reference_stack, Ic_stack, koefs = self.__solve__()

        return DICOutput(node_x, node_y, self.__input__, ref_stack=reference_stack, Ic_stack=Ic_stack), koefs
