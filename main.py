...
parser.add_argument("--task", type=str, default='calculate')
args = parser.parse_args()

cfg = Config()
save_full_name = cfg.save_path + 'data_' + str(cfg.image_times[0]) + '_' + str(cfg.image_times[1]) + '.npy'
save_viz_full_name = cfg.save_path + 'viz_' + 'data_' + str(cfg.image_times[0]) + '_' + str(cfg.image_times[1]) + '.npy'
...
if (args.task == 'calculate') :
    ...
    subset_number = subset_number_y * subset_number_x


    reference_time = 0
    initital_z_guess = 0
    z_size = len(range(0, image_z_max, cfg.z_bounce))
    analyze_result = np.zeros((cfg.sub_group_size * subset_number_y, cfg.sub_group_size * subset_number_x, z_size, 3))
    ...
    save_result_ref = np.zeros((z_size * subset_number, 3))
    save_result_def = np.zeros_like(save_result_ref)
    ...
    for i in range (0, subset_number_y) :
        for j in range (0, subset_number_x):
            ...
            for k in range(0, image_z_max, cfg.z_bounce) :
                k_bounced = k // cfg.z_bounce
                save_result_ref[subset_number_x * i + j + k_bounced * subset_number] = np.array([subset_center[0], subset_center[1], k])
                ...
                for z in range (search_z_min, search_z_max + 1) :             
                    ...
                    deformed_id = cfg.image_t_max

                    if (koefs[deformed_id - 1] < cur_koef) :
                        cur_koef = koefs[deformed_id - 1]
                        best_z = z
                ...
                save_result_def[subset_number_x * i + j + k_bounced * subset_number] = np.array([subset_center[0], subset_center[1], best_z])

                #TODO :
                # initital_z_guess = ...
                #
                #
            print('Calculating ', math.ceil(100 * (subset_number_x * i + j) / subset_number), '%')

    # Save
    np.save(save_full_name, save_result_ref)
    np.save(save_full_name, save_result_def)
    np.save(save_viz_full_name, analyze_result)

    print('TIME SPENT:', time.time() - start_time)

# Load

load_result_ref = np.load(save_full_name)
load_result_def = np.load(save_full_name)

loaded_analyze_result = np.load(save_viz_full_name)
...
plot_z = loaded_analyze_result[:, :, bounced_z_img_id, 2]
